...
function TargetPicker({ g, ovr, pending, onPick, onClose }) {
  const boardUs = g.board.map((c, i) => ({ c, i })).filter((x) => x.c);
  const handUs = g.hand.map((c, i) => ({ c, i })).filter((x) => x.c.k === "u" && x.i !== pending.handIndex);
  const Row = ({ c, onClick }) => {
...
export default function App() {
  ...
  const playCard = (i) => {
    ...
    if (eff.type === "gold") {
      setGame({ ...g, gold: g.gold + eff.n, hand: g.hand.filter((_, j) => j !== i) });
      flash(`"${src.n}": +${eff.n} vàng`);
    } else if (eff.type === "buff" && eff.tg === "all") {
      const b = g.board.map((c) => (c ? { ...c, dA: (c.dA || 0) + bA, dH: (c.dH || 0) + bH } : c));
      setGame({ ...g, board: b, hand: g.hand.filter((_, j) => j !== i) });
      flash(`"${src.n}": +${bA}/+${bH} cho toàn bộ quân trên sân${eff.temp ? " (buff tạm — nhớ gỡ sau)" : ""}`);
    } else if (eff.type === "buff" || eff.type === "double") {
      setPendingUse({ handIndex: i, eff, name: src.n, isDan, finalA: bA, finalH: bH });
    } else {
      setGame({ ...g, hand: g.hand.filter((_, j) => j !== i) });
      flash(`Đã dùng "${src.n}" — hiệu ứng đặc biệt, áp dụng thủ công theo mô tả thẻ`);
    }
  };
  const applyPending = (where, index) => {
    ...
    if (where === "board") { board = [...g.board]; board[index] = { ...tgt, dA: (tgt.dA || 0) + a, dH: (tgt.dH || 0) + h }; }
    else hand[index] = { ...tgt, dA: (tgt.dA || 0) + a, dH: (tgt.dH || 0) + h };
    hand = hand.filter((_, j) => j !== p.handIndex);
    setGame({ ...g, board, hand });
    setPendingUse(null);
    flash(`"${p.name}": +${a}/+${h} cho ${UNITS[tgt.id].n}${p.eff.temp ? " (buff tạm — nhớ gỡ sau)" : ""}`);
  };
