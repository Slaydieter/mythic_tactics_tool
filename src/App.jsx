...
/* ============ TRIỆU TẬP: điền 3 lựa chọn, app tư vấn chọn ============ */
function DiscoverSheet({ g, ovr, disc, onOpenPicker, onChoose, onClose }) {
  const cur = scoreBoard(g.board, ovr);
  const boardTags = new Set();
  ...
  const evals = disc.slots.map((id) => (id !== null ? evalCand(id) : null));
  let best = -1, bestD = -Infinity;
  evals.forEach((ev, i) => { if (ev && ev.delta > bestD) { bestD = ev.delta; best = i; } });
  const filled = disc.slots.filter((x) => x !== null).length;
  return (
    <div className="fixed inset-0 z-50 flex items-end" style={{ background: "rgba(8,10,16,0.85)" }} onClick={onClose}>
      <div className="w-full rounded-t-3xl p-5 max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()} style={{ background: COL.panel, borderTop: `2px solid ${COL.gold}` }}>
        <div className="text-sm font-extrabold mb-1" style={{ color: COL.ink }}>🔮 Triệu Tập — "{disc.name}"</div>
        <div className="text-[11px] mb-3" style={{ color: COL.mut }}>Điền 3 đơn vị game đưa ra, app sẽ chấm điểm nên chọn lá nào. Chọn xong lá đó vào Túi.</div>
        {disc.slots.map((id, i) => {
          const u = id !== null ? UNITS[id] : null;
          const ev = evals[i];
          return (
            <div key={i} className="rounded-xl p-3 mb-2" style={{ background: COL.panel2, border: `1px solid ${best === i && filled > 1 ? COL.gold : COL.line}` }}>
              {u ? (
                <>
                  <div className="flex items-center justify-between">
                    <div className="min-w-0">
                      <div className="text-xs font-bold break-words" style={{ color: COL.ink }}>
                        {best === i && filled > 1 ? "★ " : ""}{u.n}
                        <span className="ml-1.5" style={{ ...NUM, color: TRIBE_META[u.t].c }}>C{u.c} · {baseStats(ovr, id)[0][0]}/{baseStats(ovr, id)[0][1]}</span>
                      </div>
                      <div className="text-[10px] leading-snug mt-0.5" style={{ color: COL.mut }}>
                        {ev.reasons.length ? ev.reasons.join(" · ") : "giá trị chỉ số thuần"} <span style={{ ...NUM, color: COL.gold }}>(+{ev.delta} điểm)</span>
                      </div>
                    </div>
                    <button onClick={() => onChoose(i)} className="shrink-0 ml-2 px-3 py-2 rounded-lg text-[10px] font-extrabold"
                      style={{ background: COL.gold + "22", color: COL.gold, border: `1px solid ${COL.gold}44` }}>Chọn</button>
                  </div>
                </>
              ) : (
                <button onClick={() => onOpenPicker(i)} className="w-full py-2 text-xs" style={{ color: COL.mut }}>+ điền lựa chọn {i + 1}</button>
              )}
            </div>
          );
        })}
        {best >= 0 && filled > 1 && <div className="text-[11px] mb-2" style={{ color: COL.gold }}>★ Gợi ý: chọn "{UNITS[disc.slots[best]].n}" (+{evals[best].delta} điểm cho đội hình hiện tại).</div>}
        <button onClick={onClose} className="w-full py-3 rounded-xl text-sm font-bold" style={{ background: COL.panel2, color: COL.ink }}>Đóng (không chọn)</button>
      </div>
    </div>
  );
}
...
function TargetPicker({ g, ovr, pending, onPick, onClose }) {
  ...
  const boardUs = g.board.map((c, i) => ({ c, i })).filter((x) => x.c && x.i !== exB);
  const handUs = g.hand.map((c, i) => ({ c, i })).filter((x) => x.c.k === "u" && x.i !== exH);
  const Row = ({ c, onClick }) => {
...
export default function App() {
  ...
  const playCard = (i) => {
    ...
    if (eff.type === "gold") {
      ...
      setGame({ ...g, shopBuff: { a: g.shopBuff.a + bA, h: g.shopBuff.h + bH }, hand: g.hand.filter((_, j) => j !== i) });
      flash(`"${src.n}": quân trong Thánh Đền +${bA}/+${bH} (buff hiện ở card Thánh Đền — chỉnh tay nếu hết hạn)`);
  ...
  const buyShopUnit = (i) => {
    ...
    setGame({ ...g, gold: g.gold - 3, shop: { ...g.shop, units: su }, hand: [...applyHandGrow(g.hand), { k: "u", id: s.id, lv: 0, dA: g.shopBuff.a, dH: g.shopBuff.h }] });
  ...
  const tabDoihinh = (
    <div className="p-4">
      ...
      <Card title={`Túi (${g.hand.length})`}
        right={<div className="flex gap-1.5">
          <button onClick={() => setDiscover({ handIndex: -1, name: "Triệu Tập (hiệu ứng tướng / thần)", slots: [null, null, null] })}
            className="text-[10px] font-extrabold px-2.5 py-1.5 rounded-lg" style={{ background: COL.gold + "22", color: COL.gold }}>🔮 Triệu Tập</button>
          <button onClick={() => setPicker({ mode: "hand" })} className="text-[10px] font-extrabold px-2.5 py-1.5 rounded-lg" style={{ background: COL.green + "22", color: COL.green }}>+ Thêm</button>
        </div>}>
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden" style={{ background: COL.bg, color: COL.ink, maxWidth: "100vw", fontFamily: "-apple-system, 'Segoe UI', Roboto, sans-serif" }}>
      ...
      {discover && (
        <DiscoverSheet g={g} ovr={ovr} disc={discover}
          onOpenPicker={(slot) => setPicker({ mode: "discover", slot })}
          onChoose={(slot) => {
            const id = discover.slots[slot];
            let hand = applyHandGrow(g.hand);
            if (discover.handIndex >= 0) hand = hand.filter((_, j) => j !== discover.handIndex);
            hand = [...hand, { k: "u", id, lv: 0, dA: 0, dH: 0 }];
            setGame({ ...g, hand });
            setDiscover(null);
            flash(`Đã Triệu Tập "${UNITS[id].n}" vào Túi`);
          }}
