...
export default function App() {
  ...
  const endRound = (res) => { // 'W' thắng | 'L' thua | 'D' hòa
    ...
    setGame((G) => {
      ...
      return { ...G, round, wins, lives, phase, survivalPlayed, sinceUp: G.sinceUp + 1, gold: baseGold(round), shop, frozen: false };
    });
  ...
  const playCard = (i) => {
    ...
    if (eff.type === "gold") {
      ...
      flash(`"${src.n}": +${bA}/+${bH}${grant.length ? " + " + grant.join(", ") : ""} cho toàn bộ quân trên sân${eff.temp ? " (buff tạm — nhớ gỡ sau)" : ""}`);
  ...
  const applyPending = (where, index) => {
    ...
    setPendingUse(null);
    flash(`"${p.name}": +${a}/+${h}${grant.length ? " + " + grant.join(", ") : ""} cho ${UNITS[tgt.id].n}${p.eff.temp ? " (buff tạm — nhớ gỡ sau)" : ""}`);
  };
